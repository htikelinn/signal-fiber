        ...
        <div className="overflow-y-auto h-full pb-20">
          <ul className="menu p-4 w-full text-indigo-900">
            <li className="mb-2">
              <Link 
                to="/login" 
                className={`font-semibold rounded-lg hover:bg-indigo-500 hover:text-white transition-colors duration-200 ${location.pathname === '/login' ? 'bg-indigo-500 text-white' : ''}`}
                onClick={handleNavigation}
              >
            ...
            <li className="mb-2">
              <Link 
                to="/signup" 
                className={`font-semibold rounded-lg hover:bg-indigo-500 hover:text-white transition-colors duration-200 ${location.pathname === '/signup' ? 'bg-indigo-500 text-white' : ''}`}
                onClick={handleNavigation}
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" />
                ...
                <li key={index} className="mb-1">
                  {hasSubLinks ? (
                    <details open={activeIndex === index}>
                      <summary
                        className={`cursor-pointer rounded-lg font-medium hover:bg-indigo-500 hover:text-white transition-colors duration-200 ${location.pathname === linkPath ? 'bg-indigo-500 text-white' : ''}`}
                      ...
                      <ul className="ml-2 mt-1 border-l-2 border-indigo-200 pl-3">
                        {subLinks[link].map((sub, subIdx) => (
                          <li key={subIdx} className="mb-1">
                            <Link 
                              to={sub.path} 
                              className={`text-sm rounded-lg hover:bg-indigo-500 hover:text-white transition-colors duration-200 py-2 ${location.pathname === sub.path ? 'bg-indigo-500 text-white' : ''}`}
                              onClick={handleNavigation}
                            >
                    ...
                    <Link 
                      to={linkPath} 
                      className={`rounded-lg font-medium hover:bg-indigo-500 hover:text-white transition-colors duration-200 ${location.pathname === linkPath ? 'bg-indigo-500 text-white' : ''}`}
                      onClick={handleNavigation}
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
