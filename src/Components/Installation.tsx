        ...
        <div className="stats shadow bg-white">
          <div className="stat">
            ...
            <div className="stat-value text-accent">
              {rows.length > 0 
                ? (rows.reduce((sum, row) => sum + parseFloat(row.DistanceKM || 0), 0) / rows.length).toFixed(1)
                : "0.0"
              }
        ...
        <div className="stats shadow bg-white">
          <div className="stat">
            ...
            <div className="stat-value text-blue-500">
              {rows.reduce((sum, row) => sum + parseInt(row.Cores || 0), 0)}
            </div>
              ...
              {currentItems.map((row) => (
                <tr key={row.id} className="hover:bg-gray-50">
                 ...
                 <div className="flex items-center">
                  ...
                  <td>{row.DistanceKM}<span className="text-xs text-gray-500 ml-1">km</span></td>
                  <td>
                    <span className="badge badge-ghost badge-sm">{row.Type}</span>
                  </td>
                  <td>
                    <div className="flex items-center">
                      <span className="font-medium">{row.Cores}</span>
                      <div className="text-xs text-gray-500 ml-1">({row.SpareCores} spare)</div>
                    </div>
                  </td>
                  <td>
                    <span className={`badge badge-sm ${getStatusClass(row.Status)}`}>
                      {row.Status}
                    </span>
                  </td>
                  <td>{row.Township}</td>
